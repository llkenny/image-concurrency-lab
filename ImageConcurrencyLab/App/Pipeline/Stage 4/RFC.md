...
## Assumptions
Stage 4 introduces visible-first admission. Requests are enqueued as visible or prefetch, and the loader always starts visible requests first when capacity becomes available.

The scheduler reacts only to load() and task completion. Priority updates (markVisible / markPrefetch) do not immediately reschedule work. This keeps Stage 4 minimal and focused on admission policy rather than reactive scheduling.
